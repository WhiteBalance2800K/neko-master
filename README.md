...
- 删除原先规则、域名、地区、代理等页面中重复出现的排行榜式展示，减少同一批数据在多个板块反复出现。
- 删除空的「网络」页面。
- 删除「地区」作为一级页面的入口，地区改为目标分析中的属性、地图和过滤维度。
- 删除概览页中不够清晰的来源、进程、目标、规则、代理、地区摘要行。
- 删除概览页中的热门地区板块。
- 删除链路页里的实时进程板块，避免和来源页、实时连接信息重复。
- 删除自动刷新时从左到右滑动的绿色进度条，改为只保留刷新按钮旋转状态。

### 增加了什么

- 新增「链路分析仪」式的信息架构，让用户优先看到谁发起连接、访问哪里、命中什么规则、走哪个代理、是否异常。
- 新增一级导航：「概览」「链路」「目标」「来源」「系统」。
- 新增「来源」页的进程视图，并与设备视图合并展示。
- 新增进程归因能力，Clash / Surge / Agent 采集链路都支持传递、聚合和展示 `process` / `processPath`。
- 新增进程统计 API 和聚合表，支持按进程查看关联域名、IP、规则、代理。
- 新增实时连接 API，用于展示当前连接的设备、进程、协议、端口、规则、代理链、目标和速率。
- 新增 Bark 流量提醒，可按后端总流量阈值推送通知。
- 新增偏好设置中的背景颜色、上传流量颜色、下载流量颜色配置，颜色选项基于 Radix Color，并补充黑、白、灰等基础色。
- 新增 Docker Hub 镜像 `whitebalance2026/neko-master`，并持续提供 `latest` 与版本标签。

### 修改了什么

- 将产品视角从「多个统计排行榜」改为「链路分析仪」，重点展示流量从来源到目标、规则、代理的完整路径。
- 将「规则 + 代理」合并为「链路」，通过分段控件在规则命中和代理出口之间切换。
- 将「域名 + IP + 地区」合并为「目标」，作为统一的目标浏览器。
- 将「设备 + 进程」合并为「来源」，进程默认展示名称，完整路径放在详情信息中。
- 将「健康 / 后端 / 设置 / Bark 提醒」收进系统相关区域，减少一级导航分散。
- 概览页保留总流量、趋势、热门域名、热门代理，去掉不清晰的混合摘要。
- 链路页的规则链路流向默认全景展示，并增强流动粒子效果。
- 关联域名图标对方形站点 favicon 做裁切优化，避免圆形容器里显示变形。
- 全项目流量单位按 Top 流量自动统一到 MB / GB / TB，避免同一页面单位混乱。
- 修复后端断链、关机或重启后采集暂停的问题，Clash WebSocket collector 增加心跳 watchdog，可主动恢复采集。
- ClickHouse `traffic_detail` 兼容新增进程字段，旧数据默认空值，不破坏既有数据。
- 自定义背景色改为整体连续背景，减少侧边栏、顶部栏、内容区之间的缝线感。
